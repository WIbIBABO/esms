...
print('poly_tr:参数：{}, rmse_poly_tr={}, R2_poly_tr={}'.format(len_poly, rmse_poly_tr, R2_poly_tr))
print(mae/Train)
acc = 0
for i in range(len(ytrain)):
    if abs(pre_poly_tr[i] - ytrain[i])<0.5:
        acc += 1
print(acc/614)
pre_poly_te = lin_reg.predict(xtest)
rmse_poly_te = stdError_func(pre_poly_te, ytest)
R2_poly_te = R2_1_func(pre_poly_te, ytest)
print('poly_te:参数：{}, rmse_poly_te={}, R2_poly_te={}'.format(len_poly, rmse_poly_te, R2_poly_te))
acc = 0
for i in range(len(ytest)):
    if abs(pre_poly_te[i] - ytest[i])<0.5:
        acc += 1
print(acc/154)
fpr, tpr, threshold = metrics.roc_curve(ytest, pre_poly_te)
